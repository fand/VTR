...
test('packaged: bundled binary wins over dev builds', () => {
  ...
  expect(findBinary('osc-tap', env({ isPackaged: true }), fs)).toBe(bundled)
})

test('packaged: falls back to dev builds when bundle is missing', () => {
  const fs = fakeFs({ [release]: 1 })
  expect(findBinary('osc-tap', env({ isPackaged: true }), fs)).toBe(release)
})

test('dev: most recently built of release/debug wins', () => {
  expect(findBinary('osc-tap', env(), fakeFs({ [release]: 1, [debug]: 2 }))).toBe(debug)
  expect(findBinary('osc-tap', env(), fakeFs({ [release]: 3, [debug]: 2 }))).toBe(release)
})

test('dev: never picks the packaged path', () => {
  const fs = fakeFs({ [bundled]: 1 })
  expect(() => findBinary('osc-tap', env(), fs)).toThrow(/not found/)
})

test('env override wins and must exist', () => {
  const fs = fakeFs({ '/custom/tap': 1, [bundled]: 2, [release]: 3 })
  expect(findBinary('osc-tap', env({ isPackaged: true, envBin: '/custom/tap' }), fs)).toBe(
    '/custom/tap'
  )
  expect(() => findBinary('osc-tap', env({ envBin: '/gone' }), fs)).toThrow(/override not found/)
})

test('name selects the binary in every location', () => {
  const playerBundled = '/App.app/Contents/Resources/bin/vtr-player'
  const playerRelease = join('/repo/osc-editor', '../osc-tap/target/release/vtr-player')
  expect(
    findBinary('vtr-player', env({ isPackaged: true }), fakeFs({ [playerBundled]: 1, [bundled]: 1 }))
  ).toBe(playerBundled)
