...
//! the origin registry — rec handling is the tap's business, and both the
//! rec echo and the punch-in priming hang off the tap event log
//! (`tap_client`), which sees every trigger source, not just OSC.
...
pub fn spawn(sock: UdpSocket, transport: Transport, echo: Echo) -> Result<()> {
